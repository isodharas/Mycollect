...
FIX: Safe path splitting to prevent index error on unexpected paths
"""
...
def get_bins_by_priority(priority_level):
    ...
    try:
        ...
        return {
            ...
            'body': json.dumps({
                'message': 'Error filtering bins by priority',
                'error': str(e)
            })
        }
...
def lambda_handler(event, context):
    ...
    try:
        ...
        # GET /bins/{bin_id}/history - Get bin history
            ...
            # FIX: safe split with length check to prevent index errors
            parts = path.split('/')
            bin_id = path_parameters.get('bin_id') or (parts[2] if len(parts) > 2 else None)
            if not bin_id:
                return {
                    'statusCode': 400,
                    'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                    'body': json.dumps({'message': 'bin_id is required'})
                }
