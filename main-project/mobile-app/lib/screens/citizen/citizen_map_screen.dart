...
class _CitizenMapScreenState extends State<CitizenMapScreen> {
  ...
  // Real coordinates for each bin location in Homagama
  final Map<String, LatLng> _binCoordinates = {
    'BIN_001': LatLng(6.8749, 80.0014),  // Homagama North Market
    'BIN_002': LatLng(6.8712, 79.9989),  // Malapalla Junction
    'BIN_003': LatLng(6.8698, 80.0045),  // Pitipana Town
    'BIN_004': LatLng(6.8765, 80.0028),  // Thalangama Road
    'BIN_005': LatLng(6.8731, 80.0067),  // Kotikawatta Bus Stop
  };

  final Map<String, String> _binLocations = {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Column(children: [
        ...
        Padding(
          ...
          child: Row(children: [
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(LangService.t('Live Bin Map', 'සජීවී කසල බඳුන සිතියම'), style: GoogleFonts.poppins(
                  fontSize: 22, fontWeight: FontWeight.w800,
                  color: const Color(0xFF0F172A))),
              Text(LangService.t('Homagama area • Updates every 30s', 'හෝමාගම ප්‍රදේශය • සෑම තත්පර 30කට'),
                  style: GoogleFonts.poppins(fontSize: 12,
                      color: Colors.grey.shade500)),
            ])),
            // Legend
            Row(children: [
              _legendDot(const Color(0xFFDC2626), LangService.t('Critical', 'අවදානම්')),
              const SizedBox(width: 8),
              _legendDot(const Color(0xFFEA580C), LangService.t('High', 'ඉහළ')),
              const SizedBox(width: 8),
              _legendDot(const Color(0xFF16A34A), LangService.t('Low', 'අඩු')),
            ]),
          ]),
        ),
        ...
        Expanded(
          ...
          child: _loading
              ...
              : ClipRRect(
                  ...
                  child: FlutterMap(
                    options: MapOptions(
                      initialCenter: LatLng(6.8728, 80.0014),
                      initialZoom: 14.5,
        ...
        if (_selectedBin != null) _buildBinCard(_selectedBin!)
        else _buildBinList(),
      ]),
    );
  }
  ...
  Widget _buildBinCard(dynamic bin) {
    ...
    final location = _binLocations[id] ?? 'Homagama Area';
      ...
      child: Column(children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start,
              children: [
            Text(id, style: GoogleFonts.poppins(fontWeight: FontWeight.w800,
        ...
        Row(children: [
          _metricChip(Icons.air_rounded, '${gas.toInt()} PPM', 'Gas', color),
          const SizedBox(width: 8),
          _metricChip(Icons.water_drop_rounded,
              '${fill.toInt()}%', 'Fill', const Color(0xFF4A8C28)),
          const SizedBox(width: 8),
          _metricChip(Icons.health_and_safety_rounded,
              health.toStringAsFixed(1), 'Risk', color),
        ]),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(
          ...
          child: Text(LangService.t('Tap a bin on the map for details', 'විස්තර සඳහා සිතියමේ කසල බඳුනක් තෝරන්න'),
